...
#--------------------------------------------------------------------
# procedure嚗餜nit_platform_player_locations
...
lw $t0, 0($t0)
lw $t0, 0($t0)  # id of the current platform
la $t1, max_level
lw $t1, 0($t1)

slt $t2 $t0, $t1
beq $t2, 1, current_platform_id_less_than_max_level  
li $v0, 1

j check_game_level_status_done
current_platform_id_less_than_max_level:

# step 2
la $t0, life_value
lw $t0, 0($t0)
bne $t0, 0, life_value_not_equal_0
li $v0, 2

j check_game_level_status_done

life_value_not_equal_0:

li $v0, 0

check_game_level_status_done:
lw $ra, 0($sp)
addi $sp, $sp, 4
...
# ******* Your codes start here: ********
	...
	lw $t1, 0($t0)  # x_loc
	la $t2, player_size
	lw $t3, 0($t2)  # player_width
	la $t4, width
	lw $t5, 0($t4)  #screen_width
	la $t6, player_speed
	lw $t7, 0($t6)
	
	slt $t9, $t1, $zero
	beq $t9, $zero, not_exceed_left_boundary
	sw $zero, 0($t0)
	sub $t9, $zero, $t7
	sw $t9, 0($t6)
	not_exceed_left_boundary:

	add $t9, $t1, $t3
	sgt $t9, $t9, $t5
	beq $t9, $zero, not_exceed_right_boundary 
	sub $t9, $t5, $t3
	sw $t9, 0($t0)
	sub $t9, $zero, $t7
	sw $t9, 0($t6)
	not_exceed_right_boundary :
...
#       f) Get time and save the current time to "time_player_getting_hurt".  
# 4. Exit
# ********** Your codes start here ************
addi $sp, $sp, -4
sw $ra, 0($sp)
...
beq $a0, 0, decrease_life_value_procedure
beq $a0, 1, increase_life_value_procedure
j exit_life_value_change

decrease_life_value_procedure:

la $t0, life_value
lw $t1, 0($t0)  # life_value
addi $t1, $t1, -2
slt $t2, $t1, $zero
beq $t2, 0, life_value_is_not_0
addi $t1, $zero, 0
life_value_is_not_0:
sw $t1, 0($t0)
addi $a0, $t1, 0
...
lw $t2, 0($t2)
sgt $t3, $t1, $t2
beq $t3, 0, life_value_is_not_max
addi $t1, $t2, 0
life_value_is_not_max:
sw $t1, 0($t0)
addi $a0, $t1, 0
li $v0, 210
syscall
...
# ******** Your codes start here: **********
	...
	lw $t2, 0($t2)
	sw $t2, 4($t0)
	la $t3, platform_rotation_speed
	sw $zero, 0($t3)
	la $t4, current_platform_address
	sw $a0, 0($t4)

	lw $t4, 12($a0)  # platform type
	bne $t4, 0, platform_not_type_0
	addi $a0, $zero, 1
	jal update_player_life_value
	j platform_landing_action_finish
	platform_not_type_0:
	bne $t4, 1, platform_not_type_1
	jal get_time
	la $t5, time_landing_on_unstable_platform
	sw $v0, 0($t5)
	j platform_landing_action_finish
	platform_not_type_1:
	bne $t4, 2, platform_not_type_2
	la $t5, spring_platform_upward_speed
	lw $t5, 0($t5)
	add $t5, $t5, $t2
	sw $t5, 4($t0)
	j platform_landing_action_finish
	platform_not_type_2:
	bne $t4, 3, platform_not_type_3
	addi $a0, $zero, 0
	jal update_player_life_value
	j platform_landing_action_finish
	platform_not_type_3:
	bne $t4, 4, platform_not_type_4
	la $t5, base_rotation_speed
	lw $t5, 0($t5)
	sub $t5, $zero, $t5
	sw $t5, 0($t3)
	j platform_landing_action_finish
	...
	platform_landing_action_finish:
...
# step 2
la $t9, platform_size
lw $t3, 0($t9)  # platform width
lw $t4, 4($t9)  # platform height
la $t9, player_size
lw $t5, 0($t9)  # player width
lw $t6, 4($t9)  # player height

check_if_player_platform_overlap_loop:
	sgt $s0, $t1, 0
	beq $s0, 0, check_if_player_platform_overlap_loop_finish
	...
	lw $s1, 4($t0)  # platform_x
	lw $s2, 8($t0)  # platform_y
	
	slt $s3, $s2, $t8
	beq $s3, 0, platform_top_y_loc_notless_player_top_y_loc
	addi $t0, $t0, 16
	addi $t1, $t1, -1
	beq $t1, 0, platform_top_y_loc_check_done
	j check_if_player_platform_overlap_loop
	platform_top_y_loc_notless_player_top_y_loc:
	platform_top_y_loc_check_done:
	
	# iii
	
	add $s3, $t8, $t6  # player_bottom_y
	sgt $s3, $s2, $s3
	beq $s3, 0, platform_top_y_loc_notmore_player_bottom_y_loc
	...
	platform_top_y_loc_notmore_player_bottom_y_loc:
	
	add $s3, $t7, $t5  # player_right_x
	slt $s3, $s3, $s1  # result of the first condition
	add $s4, $s1, $t3  # platform_right_x
	sgt $s4, $t7, $s4  # result of the second condition
	or $s3, $s3, $s4
	beq $s3, 0, expression_notmet_for_task3_3iii
	...
	expression_notmet_for_task3_3iii:


check_if_player_platform_overlap_loop_finish:
...
la $s0, current_platform_address
lw $s1, 0($s0)
lw $s1, 0($s1) # current_platform id
lw $s2, 0($t0) # platform that landed on id
beq $s1, $s2, landed_on_the_same_platform
...
addi $t3, $zero, 0  # iterator

shift_left_platforms_loop:

sll $t4, $t3, 2

add $t5, $t0, $t4  # address where the 16(platforms) is to be stored
addi $t6, $t4, 16
add $t6, $t0, $t6  # address of 16(platforms)
lw $t6, 0($t6)  # value of 16(platforms)
sw $t6, 0($t5)
addi $t3, $t3, 1 
addi $t2, $t2, -1
bne $t2, 0, shift_left_platforms_loop

# step 5
lw $t4, -12($t5)  # value of the last platform's id
addi $t4, $t4, 1  # new id

# step 6
lw $a0, -8($t5)  # x_loc of the last platform
la $a1, width
lw $a1, 0($a1)
li $v0, 211
syscall
addi $t6, $v0, 0  # new x_loc

# step 7
lw $t7, -4($t5)  # y_loc of last platform
la $t8, platform_height_distance
lw $t8, 0($t8)
add $t7, $t7, $t8  # new y_loc

# step 8
li $v0, 212
syscall
addi $t8, $v0, 0  # new platform type
